...
const rules = [
  ...
  { code: 'A2', name: '环比分析', logic: '取最近两个有数据的月份，支出涨跌幅超过 ±20% 时给出提示' },
  { code: 'A3', name: '结余健康度', logic: '结余率 =（收入 − 支出）/ 收入；为负判定入不敷出，低于 10% 判定储蓄空间不足' },
  { code: 'A4', name: '成员预算预警', logic: '按成员月度预算与当月实际支出对比，使用率 > 100% 判定超支，≥ 80% 判定接近上限' },
  { code: 'A5', name: '支出结构集中度', logic: '占比最高的一级分类超过总支出 50% 时提示结构单一，并列出前三分类' },
  { code: 'A6', name: '商家消费集中度', logic: '单一商家占已填写商家消费的 20% 以上时提示消费集中' },
