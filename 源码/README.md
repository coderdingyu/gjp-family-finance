...
| 需求 | 功能 | 说明 |
| ---- | ---- | ---- |
| 1 | 文件上传 + Dify 智能体解析 | 图片/Excel/PDF 多文件排队解析，进度显示，解析结果先进**待确认区**由用户确认后入库；无关文件在待确认区标记原因并丢弃 |
| 4 | 智能分析升级为带规则的智能体 | 现有 8 条规则负责取数与算指标，大模型基于这些客观数字给出结论 |
| 5 | 模糊搜索 / 问答助手 | 大模型把自然语言转成受限的结构化查询，并受同一套权限收敛约束 |

悬浮球的「文件上传」入口已就位，当前点击提示"开发中"，
第二批把 `QuickAddBall.vue` 里的 `uploadReady` 置为 `true` 即可接通。
